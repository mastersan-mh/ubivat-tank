...
static void explode_common_modelaction_endframef(void * this, unsigned int imodel, char * actionname)
{
    void * parent;
    if((parent = entity_parent(this)))
    {
        player_vars_t * this_vars = entity_vars(this);
        if(this_vars->bull == this)
        {
            this_vars->bull = NULL;
