...
#define CHECK2(x, y) do{ if( x!= y) return NULL; }while(0)
...
static bool map_try_load_spawn(int fd, const char *class_string)
{
    ...
    CHECK(read(fd, &DATA.orig.x, 2), 2);
    CHECK(read(fd, &DATA.orig.y, 2), 2);
    CHECK(read(fd, &DATA.scores, 4), 4);
    CHECK(read(fd, &DATA.health, 2), 2);
    CHECK(read(fd, &DATA.armor , 2), 2);
	map_spawn_add(&DATA);
	return true;
}
...
static bool map_try_load_obj(int fd, const char * class_string)
{
    ...
    CHECK(read(fd, &DATA.orig.x,2), 2);
    CHECK(read(fd, &DATA.orig.y,2), 2);
    CHECK(read(fd, &DATA.message,65), 65);
	map_obj_add(&DATA);
	return true;
};
/*
...
int map_load(const char * mapname)
{
	...
	//чтение карты
    CHECK(read(fd, map.map, c_MAP_sx*c_MAP_sy), c_MAP_sx*c_MAP_sy);
